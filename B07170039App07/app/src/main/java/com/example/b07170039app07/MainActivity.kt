package com.example.b07170039app07
    ...
    private fun setListener() {
            ...
            if (ed_book.length() < 1 || ed_price.length() < 1 || ed_year.length() < 1)
                ...
                try {
                    ...
                    dbrw.execSQL(
                        ...
                        arrayOf(ed_book.text.toString(), ed_price.text.toString(), ed_year.text.toString())
                    )
                    showToast("新增:${ed_book.text},價格:${ed_price.text},年分:${ed_year.text}")
                    cleanEditText()
                } catch (e: Exception) {
                    showToast("新增失敗:$e")
                }
        ...
        findViewById<Button>(R.id.btn_update).setOnClickListener {
            ...
            if (ed_book.length() < 1 || ed_price.length() < 1 || ed_year.length() < 1)
                ...
                try {
                    ...
                    showToast("更新:${ed_book.text},價格:${ed_price.text},年分:${ed_year.text}")
                    cleanEditText()
                } catch (e: Exception) {
                    showToast("更新失敗:$e")
                }
        ...
        findViewById<Button>(R.id.btn_query).setOnClickListener {
            ...
            for (i in 0 until c.count) {
                //加入新資料
                items.add("書名:${c.getString(0)}\t\t\t\t 價格:${c.getInt(1)}\t\t\t\t 年分:${c.getInt(1)}")
                c.moveToNext() //移動到下一筆
            }
